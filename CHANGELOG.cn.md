...
- **修复:** 扫描时的 GATT 设备信息连接现在仅限受监控设备（或名称/MAC 与受监控设备匹配的设备），同一时间只连接一个，并对失败设备退避重试。避免周围 BLE 设备较多时与蓝牙音频等资源争抢（如音频卡顿、掉线）。
- **修复:** 受监控设备 UUID 轮转等待 MAC 关联期间，设备列表不再出现重复条目。
- **修复:** 无效 RSSI（0 或正数异常值）不再作为信号强度显示。
- **修复:** 未解析设备的灰色文字在 26 以下版本 macOS 深色模式下不再几乎不可见。
- **修复:** 设备列表菜单项拆分为独立按钮与文本元素，兼容更旧的 macOS 版本。
- **修复:** 开机启动开启失败时复选框状态回滚，不再显示未生效的勾选。
